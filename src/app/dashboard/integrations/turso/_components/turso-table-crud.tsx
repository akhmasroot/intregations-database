      ...
      editableColumns.forEach((col) => {
        ...
        if (val !== "" && val !== undefined) {
          ...
          if (type === "INTEGER" || type === "NUMERIC") {
            ...
            data[col.column_name] = val === "true" || val === "1" ? 1 : 0;
          } else {
...
export function TursoTableCrud({ tableName, schema = [] }: TursoTableCrudProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/integrations/turso/tables/${encodeURIComponent(tableName)}?${params}`
      );
  ...
  const handleInsert = async (rowData: Record<string, unknown>) => {
    const response = await fetch(
      `/api/integrations/turso/tables/${encodeURIComponent(tableName)}`,
      {
        method: "POST",
