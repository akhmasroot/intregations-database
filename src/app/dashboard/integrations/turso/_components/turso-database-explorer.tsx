...
import { TursoTableCrud } from "./turso-table-crud";
import { TursoTableCreator } from "./turso-table-creator";
import { TursoSqlEditor } from "./turso-sql-editor";
import {
  Table2,
  Search,
  RefreshCw,
...
export function TursoDatabaseExplorer() {
  ...
  const fetchTables = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/integrations/turso/tables");
  ...
  const fetchSchema = useCallback(async (tableName: string) => {
    ...
    try {
      const response = await fetch(
        `/api/integrations/turso/schema?table=${encodeURIComponent(tableName)}`
      );
      const result = await response.json();
        ...
        {selectedTable ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
            ...
            <p className="text-xs mt-1">
              Choose a table from the left panel to view and manage data
            </p>
          </div>
        )}
      </div>
