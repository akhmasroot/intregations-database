...
function RowDialog({ open, onClose, onSave, schema, initialData, mode }: RowDialogProps) {
    ...
    if (open) {
      ...
      schema.forEach((col) => {
        if (col.column_name !== "id" && col.column_name !== "created_at") {
          initial[col.column_name] = initialData
            ? String(initialData[col.column_name] ?? "")
            : "";
        }
      });
      setFormData(initial);
  ...
  const editableColumns = schema.filter(
    (col) => col.column_name !== "id" && col.column_name !== "created_at"
  );
      ...
      editableColumns.forEach((col) => {
        ...
        if (val !== "" && val !== undefined) {
          ...
          if (type.includes("int") || type.includes("serial")) {
            data[col.column_name] = parseInt(val, 10);
          } else if (type.includes("float") || type.includes("numeric") || type.includes("double")) {
            data[col.column_name] = parseFloat(val);
          } else if (type.includes("bool")) {
            data[col.column_name] = val === "true" || val === "1";
          } else if (type.includes("json")) {
            try {
              data[col.column_name] = JSON.parse(val);
            } catch {
        ...
        } else if (col.is_nullable === "YES" || col.is_nullable === true) {
...
export function TableCrud({ tableName, schema = [] }: TableCrudProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/integrations/supabase/tables/${encodeURIComponent(tableName)}?${params}`
      );
  ...
  const handleInsert = async (rowData: Record<string, unknown>) => {
    const response = await fetch(
      `/api/integrations/supabase/tables/${encodeURIComponent(tableName)}`,
      {
        method: "POST",
  ...
  const columns: DataTableColumn[] = schema.map((col) => ({
    key: col.column_name,
  ...
  // If no schema, derive columns from data
