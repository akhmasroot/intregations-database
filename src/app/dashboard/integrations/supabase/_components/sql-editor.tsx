...
export function SqlEditor({ hasServiceKey = false }: SqlEditorProps) {
  ...
  const runQuery = useCallback(async () => {
    ...
    // Safety check: warn about non-SELECT queries without service key
    ...
    if (
      ...
      toast.error(
        "Only SELECT queries are allowed without a Service Role Key. Add a service key to run write operations."
      );
      return;
    }
    ...
    setIsRunning(true);
    try {
      const response = await fetch("/api/integrations/supabase/sql", {
        method: "POST",
          ...
          <div className="flex items-center gap-2">
            {!hasServiceKey && (
              <div className="flex items-center gap-1.5 text-xs text-yellow-400">
                ...
                <span>SELECT only (no service key)</span>
              </div>
            )}
          </div>
          <div className="flex items-center gap-2">
