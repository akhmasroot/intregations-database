...
export function DatabaseExplorer({ hasServiceKey = false }: DatabaseExplorerProps) {
  ...
  const fetchTables = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/integrations/supabase/tables");
  ...
  const fetchSchema = useCallback(async (tableName: string) => {
    ...
    try {
      const response = await fetch(
        `/api/integrations/supabase/schema?table=${encodeURIComponent(tableName)}`
      );
      const result = await response.json();
          ...
          <>
            ...
            >
              ...
              <TabsContent
                ...
                className="flex-1 overflow-hidden mt-0 p-0"
              >
                <SqlEditor hasServiceKey={hasServiceKey} />
              </TabsContent>
            </Tabs>
          </>
        ) : (
