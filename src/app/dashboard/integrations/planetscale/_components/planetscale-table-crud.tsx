...
interface PlanetScaleTableCrudProps {
  tableName: string;
  schema?: SchemaColumn[];
}
...
export function PlanetScaleTableCrud({ tableName, schema = [] }: PlanetScaleTableCrudProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/integrations/planetscale/tables/${encodeURIComponent(tableName)}?${params}`);
  ...
  const handleInsert = async (rowData: Record<string, unknown>) => {
    const response = await fetch(`/api/integrations/planetscale/tables/${encodeURIComponent(tableName)}`, {
      method: "POST",
