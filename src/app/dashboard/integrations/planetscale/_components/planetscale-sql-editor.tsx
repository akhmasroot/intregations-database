...
export function PlanetScaleSqlEditor() {
  ...
  const runQuery = useCallback(async () => {
    ...
    setIsRunning(true);
    try {
      const response = await fetch("/api/integrations/planetscale/sql", {
        method: "POST",
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = "planetscale-query.csv";
    a.click();
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-border">
        ...
        <div className="flex items-center justify-between mt-3">
          <span className="text-xs text-muted-foreground">MySQL syntax (PlanetScale is MySQL-compatible)</span>
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">Ctrl+Enter to run</span>
            <Button onClick={runQuery} disabled={isRunning || !query.trim()} size="sm" className="bg-purple-600 hover:bg-purple-700">
          ...
          <>
            <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-muted/20">
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                {result.error ? (
                  <span className="text-red-400 flex items-center gap-1"><AlertCircle className="h-3.5 w-3.5" />Error</span>
                ) : (
                  <><span>{result.rowCount} rows</span><span className="flex items-center gap-1"><Clock className="h-3 w-3" />{result.executionTime}ms</span></>
                )}
              </div>
            ...
            {result.error ? (
              <div className="p-4">
                <div className="flex items-start gap-2 p-3 rounded-md border border-red-500/20 bg-red-500/5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1 font-mono">{result.error}</p>
                  </div>
                </div>
              </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>{columns.map((col) => <TableHead key={col} className="text-xs font-medium">{col}</TableHead>)}</TableRow>
                </TableHeader>
                <TableBody>
                  {result.data.map((row, i) => (
                    <TableRow key={i}>
                      {columns.map((col) => {
                        ...
                        return (
                          <TableCell key={col} className={cn("text-xs py-2 max-w-[200px] truncate", isNull && "text-muted-foreground/50 italic")}>
